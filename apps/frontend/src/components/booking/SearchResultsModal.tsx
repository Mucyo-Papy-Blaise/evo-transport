...
export function SearchResultsModal({
  ...
  const isLongDistanceRoute = distance > 400;
  ...
  useEffect(() => {
    ...
    // Direct match → show it; otherwise show all as alternatives
    setResults(shuttle ? [shuttle] : Object.values(mockShuttles));
  }, [filters]);
        ...
        <>
          ...
          >
            ...
            <div className="flex items-start justify-between px-6 py-4 border-b border-border">
              <div>
                <h2 className="text-lg font-semibold text-foreground">Available Shuttles</h2>
                {filters && (
                  <div className="flex items-center gap-1.5 mt-1 text-sm text-muted-foreground">
                    <MapPin className="h-3.5 w-3.5 text-primary shrink-0" />
                    <span className="truncate max-w-35">{filters.fromLocation}</span>
                    <ArrowRight className="h-3 w-3 shrink-0" />
                    <MapPin className="h-3.5 w-3.5 text-primary shrink-0" />
                    <span className="truncate max-w-35">{filters.toLocation}</span>
                    {distance > 0 && (
                      <span className={cn(
            ...
            {/* Long-distance notice inside modal */}
            {isLongDistanceRoute && !isLoading && (
              <div className="mx-6 mt-4 flex items-start gap-2.5 rounded-lg bg-amber-50 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-800 px-4 py-3 text-xs text-amber-700 dark:text-amber-400">
                <AlertTriangle className="h-3.5 w-3.5 mt-0.5 shrink-0 text-amber-500" />
                <p>
                  This is a long distance route ({distance} km). You can still select a shuttle —
                  you&apos;ll be asked to send a special request message on the booking page before confirming.
                </p>
              </div>
            )}
                ...
                <>
                  {results.length > 1 && (
                    <p className="text-xs text-muted-foreground pb-1">
                      No direct shuttle for this exact route. Showing all available options.
                    </p>
                  )}
                  ...
                  {results.map((shuttle, idx) => (
                    <motion.div
                      ...
                      className="group relative rounded-xl border border-border bg-card hover:border-primary/50 hover:shadow-md transition-all duration-200 cursor-pointer overflow-hidden"
                      onClick={() => onSelectResult(shuttle)}
                    >
                      {/* Seat availability stripe */}
                      <div className={cn(
                        'absolute left-0 top-0 bottom-0 w-1 rounded-l-xl',
                        shuttle.availableSeats <= 3 ? 'bg-amber-400' :
                        shuttle.availableSeats <= 8 ? 'bg-emerald-400' : 'bg-primary',
                      )} />
                      ...
                      <div className="pl-4 pr-5 py-4">
                        <div className="flex items-start gap-3">
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="flex items-start justify-between gap-2">
                              <div>
                                <p className="font-semibold text-foreground leading-tight">{shuttle.provider}</p>
                                <span className={cn(
                                  'inline-block text-xs px-2 py-0.5 rounded-full font-medium mt-0.5',
                                  vehicleTypeColor[shuttle.vehicleType] ?? 'bg-muted text-muted-foreground',
                                )}>
                                  {shuttle.vehicleType}
                                </span>
                              </div>
                            ...
                            <div className="flex items-center gap-3 mt-2.5 text-sm">
                              ...
                              <span className="text-foreground font-medium">{shuttle.arrivalTime}</span>
                              <span className="text-muted-foreground text-xs">({shuttle.duration})</span>
                              {shuttle.distance && (
                                <span className="text-muted-foreground text-xs ml-auto">{shuttle.distance} km</span>
                              )}
                            </div>
                            ...
                            <div className="flex items-center justify-between mt-2">
                              ...
                              <div className={cn(
                                ...
                                shuttle.availableSeats <= 3 ? 'text-amber-600' : 'text-emerald-600',
                              )}>
                                <Users className="h-3 w-3" />
                                {shuttle.availableSeats <= 3 ? `Only ${shuttle.availableSeats} left` : `${shuttle.availableSeats} seats`}
                              </div>
                            </div>
                          </div>
                        </div>
                        ...
                        <div className="mt-3 pt-3 border-t border-border flex items-center justify-between">
                          <p className="text-xs text-muted-foreground">
                            {isLongDistanceRoute
                              ? 'Select to continue — special request at checkout'
                              : 'Tap to select this shuttle'}
                          </p>
                          <Button
                            size="sm"
                            className="h-8 text-xs gap-1 group-hover:gap-2 transition-all"
                            onClick={(e) => { e.stopPropagation(); onSelectResult(shuttle); }}
                          >
