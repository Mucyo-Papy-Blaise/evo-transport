...
export function MapView({
  ...
  useEffect(() => {
    ...
    const map = new mapboxgl.Map({
      ...
      center: [4.3517, 50.8503], // Brussels city centre
      zoom: 8.5, // ~200 km radius view
      minZoom: 6,
      maxZoom: 14,
    ...
    map.on("load", () => {
      ...
      airportEl.appendChild(chip);
      airportEl.appendChild(dot);

      new mapboxgl.Marker({ element: airportEl, anchor: "bottom" })
        .setLngLat([4.4844, 50.9014]) // BRU — Zaventem
        .addTo(map);
    });
  ...
  useEffect(() => {
    ...
    if (from) {
      fromMarker.current = new mapboxgl.Marker({
        element: createPinElement(from.city || from.name, "emerald"),
        anchor: "bottom",
      })
  ...
  useEffect(() => {
    ...
    if (to) {
      toMarker.current = new mapboxgl.Marker({
        element: createPinElement(to.city || to.name, "rose"),
        anchor: "bottom",
      })
