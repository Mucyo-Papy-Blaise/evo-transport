...
export default function BookingPage() {
  ...
  const [longDistanceMessage, setLongDistanceMessage] = useState("");
  ...
  // Compute distance from mock data using route codes
  const routeDistance =
    selectedRoute?.fromCode && selectedRoute?.toCode
      ? getRouteDistance(selectedRoute.fromCode, selectedRoute.toCode)
      : ((selectedRoute?.shuttle as any)?.distance ?? 0);
  ...
  const validateForm = () => {
    ...
    // Validate message for long distance
    if (isLongDistanceRoute && !longDistanceMessage.trim()) {
      setMessageError(
        "Please describe your trip requirements so we can assist you.",
      );
      return false;
    }
  ...
  const handleBooking = () => {
    ...
    if (!validateForm() || totalPassengers < 1) {
      if (totalPassengers < 1) toast.error("Add at least one passenger", "");
      return;
    }
    ...
    if (isLongDistanceRoute) {
      const longDistanceData = mapFormToLongDistanceRequest(
        {
          ...commonFormData,
          distance: routeDistance,
          message: longDistanceMessage,
        },
        userInfo,
      );

      sendLongDistanceRequest.mutate(longDistanceData, {
        onSuccess: (response) => {
          sessionStorage.removeItem("selectedRoute");
          router.push(
            `/booking/success?ref=${response.bookingReference}&type=long-distance`,
          );
        },
      });
    } else {
      const bookingData = mapFormToBookingRequest(commonFormData, userInfo);
  ...
  const isPending =
    createBooking.isPending || sendLongDistanceRequest.isPending;
      ...
      <div className="container max-w-6xl mx-auto px-4">
        ...
        {isLongDistanceRoute && (
          <motion.div
            ...
            className="mb-6 flex items-start gap-3 rounded-xl border border-amber-300 bg-amber-50 dark:bg-amber-950/30 dark:border-amber-800 px-5 py-4"
          >
            ...
            <div>
              ...
              <p className="text-amber-700 dark:text-amber-400 text-sm mt-0.5">
                This journey exceeds 400 km and requires special arrangements.
                Fill in your requirements below and our team will reach out to
        ...
        >
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            {/* Long Distance Request Message — shown only when route > 400 km */}
            {isLongDistanceRoute && (
              <motion.div
                ...
                <Card className="border-2 border-amber-300 dark:border-amber-800">
                  <CardHeader>
                    <CardTitle className="flex items-center gap-2 text-amber-800 dark:text-amber-300">
                      ...
                      Special Arrangements Request
                      <span className="text-xs bg-amber-100 dark:bg-amber-900 text-amber-700 dark:text-amber-300 px-2 py-1 rounded-full font-normal">
                        Required
                      </span>
                    </CardTitle>
                    <CardDescription>
                      Long distance trips ({routeDistance} km) need custom
                      arrangements. Describe your requirements and our team will
                      contact you to confirm details and pricing.
                    </CardDescription>
                  </CardHeader>
                  <CardContent className="space-y-3">
                    <Label
                      htmlFor="longDistanceMessage"
                      className={cn(
                    ...
                    <Textarea
                      id="longDistanceMessage"
                      placeholder={`Example:\n- Preferred pickup time: 7:00 AM\n- I need a vehicle with extra luggage space\n- Travelling with elderly passengers\n- Any special stops along the way`}
                      value={longDistanceMessage}
                      onChange={(e) => {
                        setLongDistanceMessage(e.target.value);
                    ...
                    <p className="text-xs text-muted-foreground">
                      We&apos;ll review your request and reach out within 2–4
          ...
          <div className="lg:col-span-1">
            <Card className="sticky top-24">
              <CardHeader>
                <CardTitle>
                  {isLongDistanceRoute ? "Request Summary" : "Booking Summary"}
                </CardTitle>
              </CardHeader>
              <CardContent className="space-y-4">
                <div className="space-y-2">
                  <div className="flex justify-between text-sm">
                    ...
                    <span>
                      {shuttle.price.toLocaleString()} FRw × {totalPassengers}
                    </span>
                  </div>
                  {isLongDistanceRoute && (
                ...
                {isLongDistanceRoute && (
                  <div className="rounded-lg bg-amber-50 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-800 p-3 text-xs text-amber-700 dark:text-amber-400 space-y-1">
                    <p className="font-semibold">What happens next?</p>
                    <p>1. We receive your request</p>
                    <p>2. Team reviews &amp; contacts you (2–4 hrs)</p>
                    <p>3. Confirm details &amp; payment</p>
